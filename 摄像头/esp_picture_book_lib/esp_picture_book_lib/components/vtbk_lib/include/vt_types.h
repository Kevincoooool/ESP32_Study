...
typedef struct _network_parameter {
    ...
    int timeout;    /*超时时长*/
    void *mime_data;    /*mime数据/JSON data*/
...
typedef int   (*vt_recv_message)(void *queue, const void *data, int wait_time_ms, uint8_t peek_msg); /*接收消息 queue是消息队列实例,data为存放数据指针, wait_time_ms消息队列为空时最多等待的毫秒数，peek_msg【默认为0直接收取队列数据，为1时只探测数据不把数据从队列取出相当于队列数据不减少】参数详见RTOS队列函数*/
...
typedef int (*vt_http_cancle)(void *params);
...
typedef struct _recongnize_resinfo {
    ...
    /*资源库列表大小*/
    byte res_size;
    byte current_languare;
} _recongnize_resinfo_t;

typedef struct _finger_data_info {
    /*热区index*/
    int hot_index;
    /*扩展数据*/
    char *extraData;
...
/*第三方资源库的配置信息*/
typedef struct thirdrepo_info {
	char *urlPrefix;	//第三方资源音频拼接的前缀
	char *cipher; 		//第三方资源的认证信息
} thirdrepo_info_t;

typedef struct _recongnize_page_info {
    int BookID;
    int PageID;
    int PageType; /*VT_RECONGNIZE_PAGE_TYPE*/ //书页类型 1-封面 2-封里 3-扉页 4-目录 5-正文 6-辅文 7-封底里 8-封底 9-其他
    int physicalNum;       //物理页码
    int physicalSate;      //物理页状态
    char *isbn;
    char *extraData;
    char *totalExtraData;
	thirdrepo_info_t thirdRepoInfo;
...
/*手指识别回调*/
typedef int (*pfun_vt_finger_state_cb)(_finger_data_info_t *data, void *userp);

typedef enum {
    /*1: chinese 2: english*/
    E_SET_PARAMS_LANGUARE = 1,
    E_SET_PARAMS_EXTRA_DATA_LEN,

    E_SET_PARAMS_MAX = 256,

    E_GET_PARAMS_LANGUARE,
    E_GET_PARAMS_EXTRA_DATA_LEN,
} E_PARAMS_TYPE;
