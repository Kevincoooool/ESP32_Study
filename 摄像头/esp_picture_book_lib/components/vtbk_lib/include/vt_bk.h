...
typedef struct vt_bk_handle* vt_bk_handle_t;
...
void vt_bk_version(char *version);
...
* in: handle[返回的绘本实例] ops [网络请求函数和启动TASK的接口，外部实现好注册进来] dectparams [翻页算法参数]
* 0：成功 其他失败
*/
...
* 0：成功 其他失败
*/
int vt_bk_register_fingerinfo_cb(struct vt_bk_handle *handle, pfun_vt_finger_state_cb cb, void *finger_userp);
...
* 0：成功 其他失败
*/
int vt_bk_select_resource(struct vt_bk_handle *handle, pfun_vt_httpRequest_cb cb, int res_id, int targetBookId);
...
* 0：成功 其他失败
*/
int vt_bk_set_params(struct vt_bk_handle *handle, E_PARAMS_TYPE type, int value);
...
* 0：成功 其他失败
*/
int vt_bk_get_params(struct vt_bk_handle *handle, E_PARAMS_TYPE type, int *value);
 ...
 * return: 第三方资源库信息
 */
thirdrepo_info_t* vt_bk_get_thirdrepo_info(struct vt_bk_handle *handle);
