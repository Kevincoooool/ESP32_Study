...
#include "gc_ble.h"
#include "gc_tcp.h"
static QueueHandle_t gatt_msg_queue = NULL;
...
//#define SAMPLE_DEVICE_NAME          "ESP-BLE_LQ"
...
static bool ble_bypass = false; //默认非直通
bool ble_to_stm32 = false;		//默认非直通
...
/* Service */
static const uint16_t GATTS_SERVICE_UUID_TEST = 0xffe0;
static const uint16_t GATTS_CHAR_UUID_TEST_A = 0xffe1;
...
static const uint8_t char_value[] = "huafu read or write";
...
static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
	...
	case ESP_GAP_BLE_ADV_START_COMPLETE_EVT:
		...
		{
			ESP_LOGD(GATTS_TABLE_TAG, "advertising start successfully");
		}
		break;
	case ESP_GAP_BLE_ADV_STOP_COMPLETE_EVT:
		...
		{
			ESP_LOGD(GATTS_TABLE_TAG, "Stop adv successfully\n");
		}
		break;
	case ESP_GAP_BLE_UPDATE_CONN_PARAMS_EVT:
		ESP_LOGD(GATTS_TABLE_TAG, "update connection params status = %d, min_int = %d, max_int = %d,conn_int = %d,latency = %d, timeout = %d",
...
{
	ESP_LOGD(GATTS_TABLE_TAG, "prepare write, handle = %d, value len = %d", param->write.handle, param->write.len);
	...
	if (param->write.need_rsp)
	{
		...
		{
			ESP_LOGE(GATTS_TABLE_TAG, "%s, malloc_psram failed", __func__);
		}
	}
...
void example_exec_write_event_env(prepare_type_env_t *prepare_write_env, esp_ble_gatts_cb_param_t *param)
{
	...
	{
		ESP_LOGD(GATTS_TABLE_TAG, "ESP_GATT_PREP_WRITE_CANCEL");
	}
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
	...
	case ESP_GATTS_READ_EVT:
		ESP_LOGD(GATTS_TABLE_TAG, "ESP_GATTS_READ_EVT");
		printf("handle = %d,gatt_if = %d,connid=%d,handle_table[%d]\r\n", param->read.handle, gatts_if, param->read.conn_id, heart_rate_handle_table[IDX_CHAR_VAL_A]);
		break;
	case ESP_GATTS_WRITE_EVT:
		if (!param->write.is_prep)
		{
			...
			if (heart_rate_handle_table[IDX_CHAR_CFG_A] == param->write.handle && param->write.len == 2)
			{
				...
				{
					ESP_LOGD(GATTS_TABLE_TAG, "notify enable");
				}
				else if (descr_value == 0x0002)
				{
					ESP_LOGD(GATTS_TABLE_TAG, "indicate enable");
				}
				else if (descr_value == 0x0000)
				{
					ESP_LOGD(GATTS_TABLE_TAG, "notify/indicate disable ");
				}
				else
				{
	...
	case ESP_GATTS_MTU_EVT:
		ESP_LOGD(GATTS_TABLE_TAG, "ESP_GATTS_MTU_EVT, MTU %d", param->mtu.mtu);
		break;
	case ESP_GATTS_CONF_EVT:
		/* 当收到确认时，事件发生 */
		ESP_LOGD(GATTS_TABLE_TAG, "ESP_GATTS_CONF_EVT, status = %d, attr_handle %d", param->conf.status, param->conf.handle);
		break;
	case ESP_GATTS_START_EVT:
		ESP_LOGD(GATTS_TABLE_TAG, "SERVICE_START_EVT, status %d, service_handle %d", param->start.status, param->start.service_handle);
		break;
	case ESP_GATTS_CONNECT_EVT:
		ESP_LOGD(GATTS_TABLE_TAG, "ESP_GATTS_CONNECT_EVT, conn_id = %d", param->connect.conn_id);
		...
		ESP_LOGW(GATTS_TABLE_TAG, "BLE Connected!");

		break;
	case ESP_GATTS_DISCONNECT_EVT:
		ESP_LOGD(GATTS_TABLE_TAG, "ESP_GATTS_DISCONNECT_EVT, reason = 0x%x", param->disconnect.reason);
		esp_ble_gap_start_advertising(&adv_params);
		/*蓝牙断开连接*/
		BLE_Connected = false;

		ESP_LOGW(GATTS_TABLE_TAG, "BLE Disonnected!");
		break;
	case ESP_GATTS_CREAT_ATTR_TAB_EVT:
	{
		...
		{
			ESP_LOGD(GATTS_TABLE_TAG, "create attribute table successfully, the number handle = %d\n", param->add_attr_tab.num_handle);
...
void bt_init(void)
{
	...
	esp_log_level_set(GATTS_TABLE_TAG, ESP_LOG_INFO);
	esp_log_level_set("BLUETOOTH_SERVICE", ESP_LOG_INFO);
}
...
void ble_gatts_init(void)
{
	...
	gatt_msg_queue = xQueueCreate(32, sizeof(gatt_msg_t));
	if (gatt_msg_queue == 0)
	{
...
void bt_task(void *parameter)
{
	...
	if (ret)
	{
		ESP_LOGE(GATTS_TABLE_TAG, "%s initialize controller failed: %s\n", __func__, esp_err_to_name(ret));
		return;
	}
